# agnt Signal Trader

Telegram bot that parses trading signals from monitored channels, auto-executes orders on MetaTrader (Demo default), notifies the owner, and tracks channel performance.

Spec: [`docs/blueprint.md`](docs/blueprint.md).

Built on [agnt-gm.ai](https://agnt-gm.ai). The whole bot is built and refined here as pull requests across successive build passes.
